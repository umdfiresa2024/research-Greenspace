...
files<-read.table("G:/Shared drives/2024 FIRE-SA/DATA/ECOSTRESS/files.txt")
files<-files$V1

#set folder to store the downloaded files
dl_dir <- "G:/Shared drives/2024 FIRE-SA/DATA/ECOSTRESS/"                                 
setwd(dl_dir)                                                
...
for (i in 1:length(50)) {
  ...
  filename <-  tail(strsplit(files[i], "ECOSTRESS_L2_LSTE_")[[1]], n = 1) 
